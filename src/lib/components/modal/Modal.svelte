...
<style lang="postcss">
    ...
    .modal__content {
        @apply flex flex-col relative justify-between shadow-lg px-8 pt-12 pb-1 bg-white text-gray-700 pointer-events-auto border border-slate-100
    }
    ...
    .modal_actions {
        @apply flex justify-end mt-4 border-t py-2 border-gray-100 
    }

    .modal__action-close {
        @apply absolute top-2 inline-grid place-content-center rounded-full w-8 h-8 border m-0 p-0 transition-colors mt-2
    }
    ...
    .modal__action-close:hover {
        @apply text-white bg-black transition duration-300 rotate-180
    }
</style>
